...
Future<CountdownEvent?> pushEditCountdownPage(
  ...
  return Navigator.push<CountdownEvent>(
    context,
    MaterialPageRoute(builder: (_) => _EditCountdownPage(event: event)),
  );
}

class _EditCountdownPage extends StatefulWidget {
  final CountdownEvent event;
  const _EditCountdownPage({required this.event});

  @override
  State<_EditCountdownPage> createState() => _EditCountdownPageState();
}

class _EditCountdownPageState extends State<_EditCountdownPage> {
  ...
  void _submit() {
    ...
    // 对非重复的农历事件，校验转换后的公历日期是否合理
    if (_repeatCycle == RepeatCycle.none &&
        _type == CountdownType.days &&
        _targetDate.isBefore(DateTime.now().subtract(const Duration(days: 1)))) {
  ...
  String get _dateLabel {
    ...
    if (_isLunar) {
      ...
      if (lunar != null) return '农历 ${lunar.year}年${lunar.monthName}${lunar.dayName}';
    }
